...
public class StepsBackLog extends RestWSTestCase {
	...
	public void addNewAddress(String city, String country, String number, String postcode,
			String street) {
		...
		getClient().resource(baseurl + "/addresses")
				...
				.post(new Gson().toJson(addressBean));
	}
	...
	public void verifyAddressAdded() {
		...
		getWebResource("/addresses").header("Content-Type", "application/json")
				.get(ClientResponse.class);
		Reporter.log("Address id:" + jsonObject.get("id"));
		Validator.verifyThat("Adding address status code",
				getResponse().getStatus().getStatusCode(), Matchers.equalTo(200));
	}
	...
	public void addNewCard(String longNum, String expiry, String cvv) {
		...
		getClient().resource(baseurl + "/cards")
				.header("Content-Type", "application/json")
				.post(new Gson().toJson(paymentBean));
		JSONObject jsonObject = new JSONObject(getResponse().getMessageBody());
		getWebResource("/cards").header("Content-Type", "application/json")
				.get(ClientResponse.class);
		Reporter.log("Address id:" + jsonObject.get("id"));
		Validator.verifyThat("Adding card status code",
				getResponse().getStatus().getStatusCode(), Matchers.equalTo(200));
	}

	@QAFTestStep(description = "user places the order")
	public void orderPlacement() {
		getWebResource("/orders").header("Content-Type", "application/json")
				.post(ClientResponse.class);
		Validator.verifyThat("Adding card status code",
				getResponse().getStatus().getStatusCode(), Matchers.equalTo(201));
	}
	...
	public void verifyOrderPlacement() {
		...
		Validator.verifyThat("Order placement status code",
				getResponse().getStatus().getStatusCode(), Matchers.equalTo(201));
	}
	...
	public void createUser() {
		...
		getWebResource("/register").header("Content-Type", "application/json")
				.post(new Gson().toJson(userBean));
		Response response = getResponse();
		JSONObject jsonObject = new JSONObject(response.getMessageBody());
		username = userBean.getUsername();
		password = userBean.getPassword();
		userID = jsonObject.getString("id");
		Validator.verifyThat("Create User:", jsonObject.getString("id").length(),
	...
	public void addProductToCart(String item) {
		...
		getWebResource("/cart").header("Content-Type", "application/json")
				.post(jsonObject.toString());
		Validator.verifyThat("Add to cart status:",
				getResponse().getStatus().getStatusCode(), Matchers.equalTo(201));
	}
	...
	public void verifyCart() {
		...
		Validator.verifyThat("Verify cart status:",
				getResponse().getStatus().getStatusCode(), Matchers.equalTo(200));
	}
	...
	public void updateCart(String quantity, String item) {
		...
		getWebResource("/cart/update").header("Content-Type", "application/json")
				.post(ClientResponse.class, new Gson().toJson(map));
		Validator.verifyThat("Update cart status:",
				getResponse().getStatus().getStatusCode(), Matchers.equalTo(202));
	}
	...
	public void deleteProduct(String item) {
		...
		Validator.verifyThat("Delete Product status code:",
				getResponse().getStatus().getStatusCode(), Matchers.equalTo(202));
	}
